...
首先我们切换到LNP的 *Advanced (高级)* 标签

![LNP Advanced tab](/imgs/lnp_advanced.png)
...
简单来说，就是大开声音，并调到最大，前面的按钮可以控制声音的开关，后面的数字则是音量等级。
...
自动保存可以设置为 **NONE (无)**、**SEASONALL (每季度)**、**YEARLY (每年)**，设置后将会根据游戏里面流逝的时间进行自动保存，此处推荐设为 **SEASONALL (每季度)**。
